import re, sys, os, requests, time, logging
...
ordinary_handler.setFormatter(logging.Formatter('%(asctime)s - %(levelname)s - %(message)s'))
...
def get_last_pipeline_number(username, password, pod):
    ...
    if response.status_code == 200:
        ...
        ordinary_logger.error(f"Failed to connect to Jenkins API. Status code: {response.status_code}")
        sys.exit(1)
...
def extract_from_ruby(ruby_file_path):
    ...
    ordinary_logger.info(f"Task: {task_name_match.group(1) if task_name_match else 'Not found'}")
...
def run(playwright: Playwright, ruby_file_path, download_flag=False) -> None:
    ...
    ordinary_logger.info(f"Filling in build parameters. Task name: {task_name}, Service: {service_name}")
    page.locator("input[name=\"value\"]").click()
    page.locator("input[name=\"value\"]").fill(task_name)
    page.locator('div[description="Please select a service"] select[name="value"]').select_option(service_name)
    ...
    ordinary_logger.info(f"Navigating to console output for build number: {pipeline_number}")
    ...
    if debug_task_match:
        ...
        if failure_task_match:
            ...
            jenkins_logger.warning("No TASK [debug] or [failure] information found in the output.")
    ...
    if download_flag and debug_task_match:
        ...
        if file_locator:
            ...
            ordinary_logger.info(f"File '{task_name}_result.csv' downloaded successfully to {download_path}")
        else:
            ordinary_logger.warning(f"File '{task_name}_result.csv' not found at {download_url}.")
